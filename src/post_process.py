...
def get_cluster(read1, read2, read_names, cluster_type, genes_overlap, consistent, same_segment, gene_id_to_name, distance_threshold=2, min_support=1):
    ...
    for cluster_indices in sorted(split_cluster, key=lambda x: len(x), reverse=True):
        ...
        if read_support>=min_support:            
            ...
            value={'median_breakpoint_1':(read1_cluster[0][6], re.match(r"[^.]*", gene_1).group(0), chrom_1, bp1, bp1_range, mapq1, r1len, intron1), \
                                              'median_breakpoint_2':(read2_cluster[0][6], re.match(r"[^.]*", gene_2).group(0), chrom_2, bp2 , bp2_range, mapq2, r2len, intron2), \
